...
Vue 3 单页应用展示简短个人简介、技能标签、示例问题和文字聊天窗口。示例问题为硬编码预设内容。答案下方展示引用资料的标题与摘录；错误信息使用面向访客的中文提示。部署为静态资源，由 Nginx 或 Spring Boot 内嵌托管。
...
通过环境变量配置主模型和备用模型（`PRIMARY_MODEL` / `FALLBACK_MODEL`），支持 DeepSeek 和千问。调用主模型超时或报错时自动切换备用模型重试一次；双模型均失败则通过 Java 层返回"服务暂不可用"。

### 数据存储

MySQL 存储会话和问答日志。Chroma 作为向量库存储资料切片、向量、分类和来源元数据。重新导入时 drop 旧 collection 并重建，接口与具体实现隔离便于后续迁移。

## 请求数据流

1. 访客输入问题，前端调用 Java 的聊天接口。
2. Java 校验非空、长度和频率，生成或复用会话，并把请求转交 Python。
3. Python 从 Chroma 检索最相关的资料片段。
4. 如果相关性不足，Python 返回"资料不足"结果；否则将问题和片段发给主模型，失败则切换备用模型。
...
- 模型 API 超时或报错：自动切换备用模型重试；双模型均失败时返回统一的"服务暂不可用，请稍后重试"，记录内部错误摘要。
- Python 不可用或 Chroma 故障：Java 层兜底，返回统一错误提示。
...
- Python 单元测试：切分、检索排序、资料不足判定、提示词组装、模型降级逻辑。
...
使用容器化部署：前端静态资源、Java 服务、Python 服务、MySQL 和 Chroma 作为独立服务；云平台以环境变量注入模型 API 密钥及主备模型配置。对外只暴露前端和 Java API，Python 内部服务不直接开放给公网。

## 个人资料替换流程

1. 准备结构化的个人简介、技能、项目、教育经历与荣誉资料。
2. 将文件提交给 Python 导入接口或导入命令。
3. 清理旧 Chroma collection 并建立新向量索引。
...
| 变量 | 说明 |
|------|------|
| `PRIMARY_MODEL` | 主模型标识，如 `deepseek` |
| `FALLBACK_MODEL` | 备用模型标识，如 `qwen`；可不设 |
| `PRIMARY_API_KEY` | 主模型 API 密钥 |
| `FALLBACK_API_KEY` | 备用模型 API 密钥；可不设 |
| `PRIMARY_BASE_URL` | 主模型 API 地址 |
| `FALLBACK_BASE_URL` | 备用模型 API 地址；可不设 |
